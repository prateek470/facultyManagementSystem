...
describe ClassController,  :type => :controller do 
  describe 'create,destroy,update,edit and show' do
    #it 'should fail if class already exists' do
      #post :create, {:class => {:room_id => 1, :time_slot_id => 1, :day_combination_id => 3, :semester_id => 1}}    
    #end
  ...
  after :all do
    @added_room = Room.find_by(room_name: 'TEST NAME')
    @r_id = @added_room.id
    Room.delete(@r_id)
  end
end
